...
(defun list-to-bit-vector (my-list)
  (let ((my-bit-vector (make-array 0 :adjustable t :element-type 'bit)))
    (dolist (el my-list)
      (adjust-array my-bit-vector (+ 1 (length my-bit-vector)))
      (setf (aref my-bit-vector  (- (length my-bit-vector) 1)) el))
    my-bit-vector))
